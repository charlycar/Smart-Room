...
function Tiempo_coord(position) {
	...
	$.ajax({
	  ...
	  success: function(data) {
	    data.main.temp = data.main.temp - 273.15;
	    document.getElementById("weather1").innerHTML = "<img src='weather/" + data.weather[0].icon + ".png' style='width: 50px'/>" +
	                                                data.main.temp.toFixed() + " ºC";
	    document.getElementById("weather2").innerHTML = "<div style='font-size:110%; color: white;'><img src='weather/" + data.weather[0].icon + ".png' style='width: 50px'/>" +
